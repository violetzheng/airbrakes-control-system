...
arguments
    ...
    args.ConfigureForDeployment (1,1) {mustBeNumericOrLogical} = false
    ...
    si = Simulink.SimulationInput('ver1_3DOF_draft2');
    ...
    function locPostStepFcn(simTime)
        so = simulink.compiler.getSimulationOutput('ver1_3DOF_draft2');
        res = extractResults(so, prevSimTime);
        stopRequested = feval(args.OutputFcn, simTime, res);
        if stopRequested
            simulink.compiler.stopSimulation('ver1_3DOF_draft2');
        end
        prevSimTime = simTime;
    end
